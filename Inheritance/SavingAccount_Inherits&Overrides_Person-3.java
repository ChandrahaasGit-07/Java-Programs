...
Create a subclass called SavingsAccount that overrides the withdraw() method to prevent withdrawals if the account balance falls below 
one hundred.

2.2: Write a Java program where the "BankAccount" class prevents duplicate account numbers.
*/
...
class BankAccount
{
    private static HashSet<Integer> usedAccountNumbers = new HashSet<>();
    ...
    {
        if(usedAccountNumbers.contains(accuntNumber))
        {
            throw new IllegalArgumentException("Error: Account Number :"+accuntNumber+" Account Number already used");
        }
        this.balance = balance;
        this.accuntNumber = accuntNumber;
        usedAccountNumbers.add(accuntNumber);
    }
...
public class Main
{
	public static void main(String[] args) {
		...
		SavingsAccount s2 = new SavingsAccount(10000, 456);
		s2.withdraw(250);
		//Check balance
		...
		SavingsAccount s3 = new SavingsAccount(30000, 123);
		s3.withdraw(2000);
		}
